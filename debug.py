...
def filetree(root, path="", exep=[".py"]):
    ...
    if path == "":
        print("")
        print(os.path.realpath(root).split("/")[-1])  # BUG: -1 --> 0

    # Get files in current folder:
    files = sorted(os.listdir(root))
    files_filtered = [item for item in files if (not item.startswith(".") and
                                                 not os.path.splitext(item)[1] in exep)]  # BUG: exep --> path
    ...
    for i in range(nfiles):
        ...
        filetree("{:s}/{:s}".format(root, files_filtered[i]), newpath, exep)  # BUG: i --> i-1
